tokio-networking by geoffjay/claude-plugins

[Made with love by Vercel](https://vercel.com "Made with love by Vercel")[Skills](/)

[OfficialNew](/official) [Audits](/audits) [Docs](/docs)

[skills](/)/ [geoffjay](/geoffjay)/ [claude-plugins](/geoffjay/claude-plugins)/tokio-networking
...
01. **Use connection pooling** for database and HTTP connections
02. **Implement health checks** for all dependencies
03. **Add circuit breakers** for external service calls
04. **Use appropriate timeouts** for all network operations
05. **Implement retry logic** with exponential backoff
06. **Add comprehensive middleware** for logging, metrics, auth
07. **Use load balancing** for high availability
08. **Deduplicate requests** to reduce load
09. **Monitor latency** and error rates
...
[Gen Agent Trust HubPass](/geoffjay/claude-plugins/tokio-networking/security/agent-trust-hub) [SocketPass](/geoffjay/claude-plugins/tokio-networking/security/socket) [SnykWarn](/geoffjay/claude-plugins/tokio-networking/security/snyk)
