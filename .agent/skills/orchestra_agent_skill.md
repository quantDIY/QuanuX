...
1. **ISO 20022 (FIX Orchestra):** Universal protocol naming definitions generated by the pure C++ `pugixml` native standardizer. You must prefer referencing standardized Tag Names (e.g., `ClOrdID`, `TransactTime`) instead of numerical values. *Notice:* The Cython boundary (`orchestra_constants.pxd`) now contains the entirety of the parsed FIX tag library. The Python layer is 100% semantically aware of the standard FIX schema.
