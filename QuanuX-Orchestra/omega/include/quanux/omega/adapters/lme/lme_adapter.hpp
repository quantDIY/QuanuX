...
#include "quanux/omega/adapters/lme/lme_structs.hpp"
...
public:
        ...
        core::OmegaEventEnvelope& out_envelope) noexcept 
    {
        ...
        const auto* msg = reinterpret_cast<const LmeExecutionMock*>(buffer);

        if (msg->order_id == 0) {
            out_envelope.provenance.parse_status = vocab::ParseStatus::Error;
            return false;
        }
        out_envelope.identity.event_id = ids::EventId(msg->order_id);

        size_t sym_len = 0;
        while (sym_len < 6 && msg->instrument_code[sym_len] != ' ' && msg->instrument_code[sym_len] != '\0') sym_len++;
        if (sym_len > 0) {
            out_envelope.identity._backing_instrument_id = std::string(msg->instrument_code, sym_len);
        ...
        out_envelope.semantics.price = msg->price;
        ...
        if (msg->exec_type == '8') {
            char r_str[10];
            snprintf(r_str, sizeof(r_str), "%u", msg->reject_reason);
            out_envelope.extensions._backing_native_reason_code = std::string(r_str);
