...
def compile_registry(
    ...
    if not venue_xml.exists():
        ...
        venue_xml = ORCHESTRA_DIR / "repository" / "FIX.latest.xml"
        if not venue_xml.exists():
    ...
    try:
        ...
        rel_venue_xml = f"venues/{venue}/broker.xml" if venue_xml.parent.name == venue else "repository/FIX.latest.xml"
        subprocess.run(
