...
3. Remove the `infra.WatchTermination(ctx, app.Out)` call native invocation to disable signal trapping loops.
4. To revert subprocess bounded execution natively, edit `internal/exec/exec.go` to remove the `.PIPE` sliding window wrappers and `BoundedOutput` struct allocations completely replacing them with standard native `bytes.Buffer` logic.
